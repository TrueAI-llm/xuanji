...
- **插件 = MCP Server**：采用 MCP（Model Context Protocol）作为标准插件协议，直接复用整个 MCP 生态
...
│   ├── xuanji-plugin/     # MCP Client：协议适配、Server 进程管理、工具注册表
...
├── plugins/               # 官方内置 MCP Server（如 shell、http、filesystem）
...
[[mcp_server]]
name = "http"
command = "xuanji-mcp-http"

[[mcp_server]]
name = "filesystem"
command = "xuanji-mcp-filesystem"
args = ["."]                    # 限制文件系统访问范围
...
xuanji 的工具分为两类：**MCP 工具**（通过 MCP Server 提供）和**系统工具**（xuanji 内置）。所有工具在 Agent 和 Workflow 模式中统一使用，对 LLM 而言无区别——都是 Tool Schema。

### 5.1 MCP 工具（xuanji-plugin）

xuanji 作为 MCP **Host/Client**，通过 MCP 协议与 MCP Server 通信。这意味着 xuanji 可以直接加载任何兼容 MCP 协议的 Server 作为工具源，复用整个 MCP 生态。

#### 为什么全面采用 MCP

| 优势 | 说明 |
|------|------|
| **即插即用的生态** | 数百个现成的 MCP Server（filesystem、git、database、browser、fetch 等）可直接使用 |
| **标准协议** | 不需要自建插件 SDK，MCP 已有 Python/TypeScript/Rust/Go 等官方 SDK |
| **能力协商** | 标准化的握手和能力声明，避免版本不匹配 |
| **三种能力** | Tools（函数调用）、Resources（数据读取）、Prompts（提示词模板） |
| **语言无关** | 任何语言都可以实现 MCP Server |

#### MCP 通信流程

```
xuanji (MCP Host/Client)          MCP Server (插件进程)
        │                                │
        │─── initialize ────────────────▶│  能力协商
        │◀── capabilities ──────────────│
        │─── initialized ───────────────▶│  通知就绪
...
xuanji 主要使用 MCP 的以下能力：

| MCP 能力 | 用途 | 优先级 |
|----------|------|--------|
| **Tools** | 工具发现（`tools/list`）和工具调用（`tools/call`） | MVP 必须 |
| **Resources** | 数据源读取（如读取文件内容、数据库记录） | P1 |
| **Prompts** | 复用 MCP Server 提供的提示词模板 | 后续 |
| **Sampling** | MCP Server 请求 LLM 推理（如 Agent 要求 MCP Server 自主判断） | 后续 |
...
# MCP Server 注册（每个是一个独立的 MCP Server 进程）
[[mcp_server]]
name = "shell"
command = "xuanji-mcp-shell"           # 可执行文件（需在 PATH 中）
# args = []                              # 可选，传给 Server 的参数
# env = { }                              # 可选，传给 Server 的环境变量
...
name = "filesystem"
command = "xuanji-mcp-filesystem"
args = ["/home/user/projects"]          # 限制文件系统访问范围
...
| Server | 提供的 Tools | 说明 |
|--------|-------------|------|
| **xuanji-mcp-shell** | `shell.run` | Shell 命令执行 |
| **xuanji-mcp-http** | `http.get`, `http.post`, `http.put`, `http.delete` | HTTP 请求 |
| **xuanji-mcp-filesystem** | `fs.read`, `fs.write`, `fs.list`, `fs.copy`, `fs.move`, `fs.delete` | 文件操作 |

这三个 Server 用 Rust 编写，随 xuanji 一起发布。

#### MCP 工具到 LLM Tool Schema 的映射

MCP 的 `tools/list` 返回的工具定义自动转换为 LLM 的 Tool Schema：

```
MCP Tool Definition              LLM Tool Schema (OpenAI/Anthropic)
─────────────────                ──────────────────────────────────
{                                {
...
映射在 xuanji-llm 的适配层完成，各 Provider 的格式差异由适配层处理。

#### MCP 进程管理

| 策略 | 说明 |
|------|------|
| 懒启动 | 注册时不启动，首次 `tools/call` 或 `tools/list` 时才启动 |
| 进程复用 | 单请求串行复用同一进程（MCP 标准行为） |
| 并发扩展 | DAG 并行调用同 Server 时，自动启动多进程实例 |
| 空闲回收 | 超过 N 分钟未使用自动关闭（默认 10 分钟） |
| 优雅退出 | 发送 MCP shutdown 通知，5s 超时后强杀 |
| 输出限制 | 单次 `tools/call` 结果最大 1MB（可配置） |
| 日志 | MCP Server 的 stderr 输出捕获到 tracing 日志系统 |

#### MCP 工具调用结果映射

MCP `tools/call` 返回的结果映射为 xuanji 的 `ToolResult`：

```rust
impl From<McpToolResult> for ToolResult {
    fn from(result: McpToolResult) -> Self {
        match result {
            // MCP 成功结果：content 数组中提取文本
            McpToolResult::Success { content } => {
                let text = content
                    .into_iter()
                    .filter_map(|c| match c {
                        McpContent::Text { text } => Some(text),
                        _ => None,  // image 等其他类型暂忽略
                    })
                    .collect::<Vec<_>>()
                    .join("\n");
                ToolResult {
            ...
            // MCP 错误结果
            McpToolResult::Error { message, code } => {
                ToolResult {
                    tool_name: /* caller sets */,
                    result: format!("MCP Error (code {:?}): {}", code, message),
                    success: false,
                }
            }
        }
...
使用官方 MCP SDK 开发，以 TypeScript 为例：
...
server.tool("greet", "打招呼", { name: z.string() }, async ({ name }) => ({
...
系统工具由 xuanji 内部直接实现，不经过 MCP 协议层，但同样注册为 Tool Schema 暴露给 LLM。
...
系统工具的调用路径：Agent/Workflow → 直接调用对应 crate API → 返回结果。不经过 MCP IPC。

系统工具和 MCP 工具在 Tool Registry 中统一注册，Agent/Workflow 引擎不区分来源。

### 5.3 工具注册表（Tool Registry）

统一的工具注册中心，聚合 MCP 工具和系统工具：

```rust
struct ToolRegistry {
    system_tools: HashMap<String, SystemToolDef>,
    mcp_tools: HashMap<String, McpToolDef>,  // 从 MCP Server 的 tools/list 获取
}

impl ToolRegistry {
    /// 获取所有可用工具的 Schema（给 LLM 用）
    fn all_tool_schemas(&self) -> Vec<ToolSchema>;

    /// 按名称查找工具，返回调度路径
    fn get(&self, name: &str) -> ToolRef;

    /// 懒加载：首次调用某 MCP Server 的工具时，启动 Server 并注册其 tools
    async fn ensure_loaded(&mut self, server_name: &str) -> Result<()>;
}

enum ToolRef {
    System(SystemToolDef),
    Mcp { server_name: String, tool: McpToolDef },
}
```
...
# === MCP Server 管理 ===
xuanji mcp list                       # 列出已注册的 MCP Server 及其工具
xuanji mcp info <name>                # 查看 MCP Server 详情和提供的工具
xuanji mcp call <server> <tool> [args] # 直接调用 MCP 工具（调试用）
...
3. **底层 CLI 接口的脆弱性** → 已通过 MCP 标准协议 + 能力协商解决（xuanji 不绑定任何底层工具的私有接口）
